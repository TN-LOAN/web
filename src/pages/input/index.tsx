...
function InputPage() {
  ...
  const calculateLoanAmount = (salary: number, debtexpenses: number): number => {
    const loanAmount = ((salary * 0.4) - debtexpenses) * 150;
    return loanAmount > 0 ? loanAmount : 0; // Ensure loanAmount is not negative
  };
    ...
    <PageLayout className='bg-[#F5F5F5]'>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="p-6 bg-white  rounded-lg w-[1065px] h-[620px] mx-auto mt-2.5 grid grid-cols-2 gap-x-4 gap-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormMessage />
                    {loanAmountError && <p className="text-red-500 text-sm mt-2">{loanAmountError}</p>}
                  </FormItem>
                )}
              />
            </div>
